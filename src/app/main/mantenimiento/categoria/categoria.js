...
function Categoria() {
	const Excel = listaexcel => {
		if (listaexcel.length > 0) {
			...
			printExcel(listaexcel, `Informe de Categorias`);
		} else {
			dispatch(
				showMessage({
			...
			axios(
				...
				.then(res => {
					...
					if (response.status === 'ok') {
						...
						dispatch(
							showMessage({
								message: `Error al agregar una categoria `,
								variant: 'error',
								autoHideDuration: 5000,
								anchorOrigin: {
									vertical: 'top', // top bottom
	...
	const editarCategoria = e => {
		...
		if (data.nombre.trim().length > 0) {
			axios(
				{
					url: `${serverapi}/actualizacategoria`,
					method: 'POST',
					data,
					options
				},
				...
				.then(res => {
					...
					if (response.status === 'ok') {
						...
						dispatch(
							showMessage({
								message: `Error al actualizar una categoria `,
								variant: 'error',
								autoHideDuration: 5000,
								anchorOrigin: {
									vertical: 'top', // top bottom
				...
				<>
					...
					>
						<Fade in={open}>
							<Box sx={stylemodal}>
								<div>
									<h2 style={{ margin: '1rem', fontWeight: 'bold' }}>Nueva Categoria</h2>
								</div>
					...
					>
						<Fade in={open1}>
							<Box sx={stylemodal}>
								<div>
									<h2 style={{ margin: '1rem', fontWeight: 'bold' }}>Editar Categoria</h2>
								</div>
