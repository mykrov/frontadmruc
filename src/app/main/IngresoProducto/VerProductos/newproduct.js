...
import { KeyboardReturn, Save, SearchOutlined, PlaylistAdd } from '@material-ui/icons';
...
function NewProduct(props) {
	...
	const getProveedor = async () => {
		let response = await axios(`${serverapi}/proveedor`);
	...
	const [disven, setDisVen] = React.useState(true);
	const handlesetDisVen = e => {
		setDisVen(e.target.checked);
	};
	// GRABA IVA
	const [grabaiva, setGrabaIva] = React.useState(true);
	...
	// %DES SUG ADICIONAL
	const [dessub, setDesSub] = React.useState('0');
	const handlesetDesSub = e => {
		setDesSub(e.target.value);
	};
	// STOCK MINIMO
	const [stockmin, setStockMin] = React.useState('0');
	const handlesetStockMin = e => {
		setStockMin(e.target.value);
	};
	// STOCK MAXIMO
	const [stockmax, setStockMax] = React.useState('0');
	...
	// UTIL VENTA
	const [utilventa, setUtilVenta] = React.useState('0');
	const handlesetUtilVenta = e => {
		setUtilVenta(e.target.value);
	};
	// UTIL MINIMO
	const [utilmin, setUtilMin] = React.useState('0');
				...
				<>
					...
					>
						...
						>
							...
							<div className="none">
								<PlaylistAdd style={{ fontWeight: 'bold', fontSize: '4rem' }} />
								<h2 style={headers}>Nuevo Producto</h2>
							</div>
						</div>
